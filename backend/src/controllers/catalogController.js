  ...
  return {
    create: async (req, res, store) => {
      try {
        ...
        return res.status(500).json({ error: err.message || "Failed to create catalog item" });
      }
    },
    update: async (req, res, store) => {
      try {
        ...
        return res.status(500).json({ error: err.message || "Failed to update catalog item" });
      }
    },
    remove: async (req, res, store) => {
      try {
        ...
        return res.status(500).json({ error: err.message || "Failed to delete catalog item" });
      }
    },
  };
}
