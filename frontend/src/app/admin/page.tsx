...
export default function AdminPage() {
  ...
  async function uploadImagesToCloudinary(files: File[]): Promise<string[]> {
    ...
    return (result.images || []).map((image: { url: string }) => image.url);
  }
  ...
  async function handleDeleteService(id: string, title: string) {
    ...
    try {
      await fetch(`/api/catalog/services/${id}`, { method: "DELETE" }).then((response) => {
  ...
  async function handleDeleteCat(cat: Category) {
    ...
    try {
      await fetch(`/api/catalog/categories/${cat.id}`, { method: "DELETE" }).then((response) => {
        if (!response.ok) throw new Error(`Request failed: ${response.status}`);
      });
  ...
  async function handleDeleteHall(id: string) {
    ...
    try {
      await fetch(`/api/catalog/halls/${id}`, { method: "DELETE" }).then((response) => {
        if (!response.ok) throw new Error(`Request failed: ${response.status}`);
      });
  ...
  function openEditHall(hall: Hall) {
    ...
    setHallServices([]);
    fetch(`/api/catalog/hall-services/${hall.id}`)
      .then((res) => {
  ...
  async function handleSaveHall() {
    if (!hallForm.name || !hallForm.capacity) {
      toast.error("Name and capacity are required"); return;
    }
    ...
    try {
      ...
      const payload: Partial<Hall> = {
        ...
        price_per_day: 0,
        price_morning: undefined,
        price_evening: undefined,
      ...
      if (editHall) {
        ...
        await fetch("/api/catalog/hall-services", {
          method: "POST",
