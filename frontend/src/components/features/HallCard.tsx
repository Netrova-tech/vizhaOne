...
import { formatPrice, formatPriceOrQuote, hasVisiblePrice } from "@/lib/utils";
...
export function HallCard({ hall, onSelect, isSelected, onEdit, onDelete }: HallCardProps) {
  ...
  const slotPrices = [
    { label: "Morning", value: hall.price_morning },
    { label: "Evening", value: hall.price_evening },
  ].filter((item) => hasVisiblePrice(item.value));
      ...
      <div className="p-4 sm:p-4">
        ...
        <div className="mb-3 rounded-2xl bg-gray-50 px-3 py-2 border border-gray-100">
          ...
          <p className={`text-sm font-bold ${hasVisiblePrice(displayPrice) ? "text-[#be123c]" : "text-gray-500"}`}>
            {hasVisiblePrice(hall.price_per_day) ? `${formatPrice(hall.price_per_day)} / day` : formatPriceOrQuote(displayPrice)}
          </p>
          {slotPrices.length > 0 && (
            <div className="mt-1.5 flex flex-wrap gap-1.5">
              {slotPrices.map((item) => (
                <span key={item.label} className="rounded-full bg-white px-2 py-0.5 text-[10px] font-semibold text-gray-600 border border-gray-100">
